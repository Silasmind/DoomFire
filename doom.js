...
function updateFireIntensityPerPixel(currentPixel){
    ...
    const belowPixelfireIntensity= fireArray[belowPixel]
    const newFireIntensity= belowPixelfireIntensity- decay>= 0 ? belowPixelfireIntensity-decay:0
    fireArray[currentPixel- decay]= newFireIntensity
}
...
function renderFire(){
    ...
    for( let row =0; row < fireHeight; row++){
        ...
        for ( let column= 0; column < fireWidth; column++){
            ...
            if( debug=== true){html+='<td>'
            html+= `<div class= "indice-index">${pixelIndex}</div>`   
            html+= fireIntensity        
            html+='</td>'} else{ 
